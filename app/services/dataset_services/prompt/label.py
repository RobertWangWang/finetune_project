def get_label_prompt(text, global_prompt=None, domain_tree_prompt=None):
    if global_prompt:
        global_prompt = f"- 在后续的任务中，你务必遵循这样的规则：{global_prompt}"
    if domain_tree_prompt:
        domain_tree_prompt = f"- 在生成标签时，你务必遵循这样的规则：{domain_tree_prompt}"
...
{domain_tree_prompt if domain_tree_prompt else ''}
